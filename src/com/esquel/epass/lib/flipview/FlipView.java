...
import com.esquel.epass.lib.flipview.Recycler.Scrap;
...
public class FlipView extends FrameLayout {
    ...
    private static final int PEAK_ANIM_DURATION = 400;// in ms
    private static final int MAX_SINGLE_PAGE_FLIP_ANIM_DURATION = 600;// in ms

    // for normalizing width/height
    private static final int FLIP_DISTANCE_PER_PAGE = 180;
    private static final int MAX_SHADOW_ALPHA = 225;// out of 255
    private static final int MAX_SHADE_ALPHA = 225;// out of 255
    private static final int MAX_SHINE_ALPHA = 50;// out of 255
    ...
    public FlipView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mIsFlippingVertically = a.getInt(R.styleable.FlipView_orientation_flip, VERTICAL_FLIP) == VERTICAL_FLIP;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_CANCEL:
            if (mIsFlipping) {
                ...
                endFlip();
                if (mOnOverFlipListener != null) {
                    mOnOverFlipListener.release(mCurrentPageIndex);
                }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        // // Log.log("ttime"+(System.currentTimeMillis()-t));
        // }
    ...
    private void setDrawWithLayer(View v, boolean drawWithLayer) {
        if (is18API) {
            if (isHardwareAccelerated()) {
                ...
                } else if (v.getLayerType() != LAYER_TYPE_SOFTWARE && drawWithLayer) {
                    v.setLayerType(LAYER_TYPE_SOFTWARE, null);
                } else if (v.getLayerType() != LAYER_TYPE_NONE && !drawWithLayer) {
                    v.setLayerType(LAYER_TYPE_NONE, null);
                }
     ...
     * @return The duration for a flip, bigger deltaFlipDistance = longer duration. The increase if duration gets smaller for bigger values of deltaFlipDistance.
     */
    ...
    /**
     * 
     * @return true if the view is flipping vertically, can only be set via xml attribute "orientation"
     */
    ...
    /**
     * Set the overflip mode of the flipview. GLOW is the standard seen in all andriod lists. RUBBER_BAND is more like iOS lists which list you flip past the first/last page but adding friction, like
     * a rubber band.
     * 
     * @param overFlipMode
     */
    ...
    /**
     * @param emptyView
     *            The view to show when either no adapter is set or the adapter has no items. This should be a view already in the view hierarchy which the FlipView will set the visibility of.
     */
    ...
    public void set18API(boolean is18API) {
        this.is18API = is18API;
    }
